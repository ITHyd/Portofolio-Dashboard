        ...
        <table className="w-full min-w-[1350px] text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wider text-ink-subtle">
              ...
              <th className="pb-3 pr-5 whitespace-nowrap">Contract £</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Rev Plan MTD</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Rev Actual MTD</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Margin Forecast %</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Pipeline Stage</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Pipeline £</th>
              <th className="pb-3 pr-5 whitespace-nowrap">Repeat / New</th>
                ...
                <tr key={p.id} className="border-t border-bg-border/60">
                  ...
                  {(["contract_value_gbp", "revenue_plan_mtd", "revenue_actual_mtd", "margin_forecast_pct"] as const).map((f) => (
                    <td key={f} className="py-3 pr-5">
                      <input type="number" className="input w-32" value={(r?.[f] as number | null) ?? ""} onChange={(e) => patch(p.id, { [f]: e.target.value === "" ? null : Number(e.target.value) } as any)} />
                    </td>
                  ))}
