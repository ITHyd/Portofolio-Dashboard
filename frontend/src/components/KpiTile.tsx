...
export function KpiTile({ label, value, suffix, trend, accent = "indigo", delay = 0, pulse, helper, info }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="text-[0.72rem] font-bold uppercase tracking-[0.18em] text-ink-subtle" title={info}>
          {label}
        </div>
        {info && (
          <button
      ...
      <div className="mt-2 flex items-end gap-1">
        <motion.div className={cn("font-numeric text-3xl font-semibold tracking-[-0.04em]", a.text)}>
          {numeric === null ? value : <motion.span>{rounded}</motion.span>}
        </motion.div>
        {suffix && <div className="pb-1 text-sm text-ink-muted">{suffix}</div>}
      </div>
      {helper && <div className="mt-2 text-xs leading-relaxed text-ink-muted">{helper}</div>}
